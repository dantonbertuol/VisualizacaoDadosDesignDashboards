...
def main():
    ...
    st.sidebar.markdown(
        "<h1 style='text-align: center; color: #baccee;'><strong><u>Monitoramento de Casos de Covid-19</u></strong></h1>", unsafe_allow_html=True)
    ...
    covid1 = df.drop(columns=['CountryCode', 'Slug',
                     'Date', 'Premium'], axis=1)
    ...
    dfn = dfn.groupby('Country')['TotalConfirmed', 'TotalDeaths', 'TotalRecovered', 'ActiveCases'].sum(
    ).sort_values(by='TotalConfirmed', ascending=False)
    dfn.style.background_gradient(cmap='Oranges')
    dfc = covid1.groupby('Country')['TotalConfirmed', 'TotalDeaths', 'TotalRecovered', 'ActiveCases'].max(
    ...
    # Caixa de seleção
    select = st.sidebar.selectbox('Escolha o Tipo de Mapa', [
                                  'Casos Confirmados', 'Casos Recuperados', 'Casos Ativos', 'Mortes'], key='1')
    ...
    if not st.sidebar.checkbox("Ocultar Gráfico de Barras", False):
        ...
        if select == "Casos Confirmados":
            ...
            fig = px.bar(dfc.head(10), y='TotalConfirmed',
                         x='Country', color='Country', height=400)
            ...
            fig = px.bar(dfc.head(10), y='TotalRecovered',
                         x='Country', color='Country', height=400)
            ...
            fig = px.bar(dfc.head(10), y='ActiveCases',
                         x='Country', color='Country', height=400)
            ...
            fig = px.bar(dfc.head(10), y='TotalDeaths',
                         x='Country', color='Country', height=400)
    ...
    if not st.sidebar.checkbox("Ocultar Gráfico 3D", True):
        ...
        fig = px.scatter_3d(dfc.head(10), x='TotalConfirmed',
                            y='TotalDeaths', z='TotalRecovered', color='Country')
        fig.update_layout(
            title='Gráfico 3D do Total de Casos, Total de Mortes e Total de Recuperados dos 20 Principais Países Afetados Neste Momento')
        st.plotly_chart(fig)
