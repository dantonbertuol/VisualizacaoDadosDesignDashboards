...
server <- function(input, output) {
  ...
  # Títulos
  TitleInput <- eventReactive(input$update,
    {
      switch(input$dataset,
    ...
    new_data2 <- new_data2 %>%
      ...
      mutate(color = if_else(year - 1 == lag(year),
        if_else(n > lag(n), "Increase",
          if_else(n == lag(n), "Same", "Decrease")
        ), "Increase"
      ))
    new_data2$color[1] <- "Increase"
    new_data2v2 <- dataset[!(dataset$kills_total < input$kills), ]
    new_data2v2 <- new_data2v2 %>%
      group_by(year) %>%
      tally()
    ymax <- max(new_data2v2$n)

    # Criação do gráfico
    ggplot(new_data2, aes(x = year, y = n)) +
      geom_col(color = "#07415c", fill = "#5cbdb7", width = 1) +
      labs(title = Title) +
      ylab(paste(Title)) +
      xlab("Year") +
      theme_bw() +
  ...
  output$view <- renderTable({
    ...
    new_data3 <- new_data3 %>%
      select(1, attack_count, kills_total, kills_mean) %>%
