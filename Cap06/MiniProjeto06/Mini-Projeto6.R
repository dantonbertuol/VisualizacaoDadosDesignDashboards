...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    collapsed = T,
    sidebarMenu(
      menuItem("Correlação", tabName = "corr", selected = T),
      menuItem("Estatística 2", tabName = "ols"),
      ...
      tabItem(
        ...
        fluidRow(
          column(
            width = 4,
            box(
              width = NULL, title = "Coeficiente de Correlação", collapsible = F,
              solidHeader = F,
...
server <- function(input, output, session) {
  ...
  observeEvent(input$solution, {
    ...
    output$cor_detail1 <- renderUI({
      withMathJax(helpText("A primeira etapa é calcular a covariância entre X e Y.
    ...
    output$cor_detail2 <- renderUI({
      HTML("<p>A variância é calculada como o desvio de cada observação dentro de uma variável da média dessa variável dividido pelo
    ...
    output$cor_detail3 <- renderUI({
      HTML(paste0("<p>Primeiro, calculamos os valores médios de X e Y, que são:</p>
    ...
    output$cor_detail4 <- renderUI({
      HTML(paste0(
        "<p>Em seguida, calculamos os desvios de cada observação dos valores médios.
    ...
    output$cor_detail5 <- renderUI({
      HTML(paste0("<p>Na próxima etapa, multiplicamos o desvio para cada uma das observações em nossos dados.</p>
    ...
    output$cor_detail7 <- renderUI({
      withMathJax(helpText("Agora temos a covariância - mas, na verdade, queremos a correlação! Para
    ...
    output$cor_detail8 <- renderUI({
      HTML(paste0("<p>Em nosso exemplo, a variância de X é: ", round(var(isolate(vals$cordata[, c("X")])), digits = 3), " e a variância de Y é: ", round(var(isolate(vals$cordata[, c("Y")])), digits = 3), "</strong></p>
    ...
    output$cor_detail10 <- renderUI({
      HTML(paste0(
        "<p>Agora que temos o coeficiente de correlação, também queremos saber: Obtivemos esse resultado simplesmente por acaso?
       ...
       <p> <math> <msub><mi>t</mi><mi>r</mi></msub> <mo>=</mo>  <mfrac><mrow> <mi>r</mi><msqrt><mi>n</mi><mo>-</mo><mn>2</mn></msqrt> </mrow> <mrow> <msqrt><mn>1</mn><mo>-</mo><msup><mi>r</mi><mn>2</mn></sup></msqrt></mrow></mfrac></math></p> # nolint
       ...
       <p> <math> <msub><mi>t</mi><mi>r</mi></msub> <mo>=</mo>  <mfrac><mrow><mn>", isolate(round(cor(vals$cordata$X, vals$cordata$Y,
          method = "pearson"
        ), digits = 2)), "</mn><msqrt><mn>10</mn><mo>-</mo><mn>2</mn></msqrt> </mrow> <mrow> <msqrt><mn>1</mn><mo>-</mo><msup><mn>", isolate(round(cor(vals$cordata$X, vals$cordata$Y,
          method = "pearson"
        ), digits = 2)), "</mn><mn>2</mn></sup></msqrt></mrow></mfrac> <mo>=</mo><mn>",
        round(isolate(round(cor(vals$cordata$X, vals$cordata$Y,
          method = "pearson"
        ), digits = 2)) * sqrt(10 - 2) / (sqrt(1 - (isolate(round(cor(vals$cordata$X, vals$cordata$Y,
          method = "pearson"
        ), digits = 2)))^2)), digits = 3), "</mn></math></p>
